...
// Environment variables are loaded via shell
...
async function main() {
  ...
  // Use a condition that is always true to delete everything
  const { error: deleteError } = await supabase.from("skills").delete().neq("id", "-1");
